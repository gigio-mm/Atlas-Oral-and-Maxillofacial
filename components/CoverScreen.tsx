...
export default function CoverScreen({ onStart, onContinueAtlas, onViewLastResult, hasSavedQuiz = false }: CoverScreenProps) {
  const [hasSavedAtlasPosition, setHasSavedAtlasPosition] = useState(false);

  useEffect(() => {
    const checkSavedPosition = window.setTimeout(() => {
      setHasSavedAtlasPosition(Boolean(window.localStorage.getItem('atlas:last-muscle-id')));
    }, 0);

    return () => window.clearTimeout(checkSavedPosition);
  }, []);
    ...
    <div className="min-h-screen w-full bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-4 sm:p-6 md:p-8 animate-in fade-in duration-500 overflow-y-auto">
      <div className="max-w-4xl w-full flex flex-col items-center text-center space-y-6 sm:space-y-8 my-auto py-6">
        ...
        <div className="pt-2 sm:pt-4 flex flex-col items-center gap-3">
          ...
          {(hasSavedAtlasPosition || hasSavedQuiz) && (
            <div className="flex flex-wrap items-center justify-center gap-2">
              {hasSavedAtlasPosition && onContinueAtlas && (
                <button
                  type="button"
                  onClick={onContinueAtlas}
                  className="inline-flex items-center justify-center rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs font-semibold text-slate-300 transition-all duration-300 hover:border-blue-400/30 hover:bg-blue-500/10 hover:text-blue-200 active:scale-95"
                >
                  Continuar no Atlas
                </button>
              )}
              {hasSavedQuiz && onViewLastResult && (
                <button
                  ...
                  className="inline-flex items-center justify-center rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs font-semibold text-slate-300 transition-all duration-300 hover:border-white/25 hover:bg-white/10 hover:text-white active:scale-95"
                >
